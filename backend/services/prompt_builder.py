...
def _env_int(name: str, default: int) -> int:
    """读环境变量整数，非法值回退默认，避免坏配置导致启动/请求失败。"""
    try:
...
# ---- 上下文预算（可通过环境变量调整）----
# 历史上这些值被硬编码得非常保守（单条消息 1400 字符、tool_result 300 字符），
# 导致有效上下文远小于上游实际窗口（~150K token）：稍长的单条输入就被
# 截成 "...[truncated]"，模型看不到后半段内容。
# 现在统一提到接近上游窗口的水平，并允许用环境变量按需下调。
CTX_MAX_CHARS_WITH_TOOLS = _env_int("QWEN_CTX_MAX_CHARS_TOOLS", 400_000)
CTX_MAX_CHARS_PLAIN = _env_int("QWEN_CTX_MAX_CHARS", 560_000)
CTX_MSG_MAX_CHARS = _env_int("QWEN_CTX_MSG_MAX_CHARS", 200_000)
CTX_MSG_MAX_CHARS_ASSISTANT = _env_int("QWEN_CTX_MSG_MAX_CHARS_ASSISTANT", 20_000)
CTX_TOOL_RESULT_MAX_CHARS = _env_int("QWEN_CTX_TOOL_RESULT_MAX_CHARS", 24_000)
CTX_MAX_HISTORY_TURNS = _env_int("QWEN_CTX_MAX_HISTORY_TURNS", 15)
# 首条/最新一条 user 消息的"任务回顾"摘要长度（原为 800/900，长任务描述会被砍头）
CTX_TASK_RECAP_CHARS = _env_int("QWEN_CTX_TASK_RECAP_CHARS", 8_000)
CTX_LATEST_RECAP_CHARS = _env_int("QWEN_CTX_LATEST_RECAP_CHARS", 12_000)
# system prompt 保留长度（原为硬编码 2000）
CTX_SYSTEM_PROMPT_CHARS = _env_int("QWEN_CTX_SYSTEM_PROMPT_CHARS", 20_000)
...
def build_prompt_with_tools(system_prompt: str, messages: list, tools: list, *, client_profile: str = OPENCLAW_OPENAI_PROFILE) -> str:
    ...
    MAX_HISTORY_TURNS = CTX_MAX_HISTORY_TURNS  # 最近 N 轮 = 2N 条消息。浏览器/长工具链场景下 5 轮不够用，会"失忆重来"
    ...
    MAX_CHARS = CTX_MAX_CHARS_WITH_TOOLS if tools else CTX_MAX_CHARS_PLAIN
    sys_part = "" if tools and client_profile == CLAUDE_CODE_OPENAI_PROFILE else (f"<system>\n{system_prompt[:CTX_SYSTEM_PROMPT_CHARS]}\n</system>" if system_prompt else "")
    ...
    for msg in reversed(messages):
        ...
        if role == "tool":
            ...
            tool_result_limit = CTX_TOOL_RESULT_MAX_CHARS if (client_profile == CLAUDE_CODE_OPENAI_PROFILE and tools) else max(CTX_TOOL_RESULT_MAX_CHARS // 4, 4000)
        ...
        # 单条消息长度上限。
        # 关键：最新一条（reversed 遍历的第 0 条）就是用户当前提问，绝不能截断，
        # 否则长文输入会被砍掉后半段（旧行为：1400 字符即截断，等效上下文极小）。
        is_latest = msg_count == 0
        if client_profile == CLAUDE_CODE_OPENAI_PROFILE and tools:
            if is_tool_result:
                max_len = CTX_TOOL_RESULT_MAX_CHARS
            elif role == "assistant":
                max_len = CTX_MSG_MAX_CHARS_ASSISTANT
            else:
                max_len = CTX_MSG_MAX_CHARS
        else:
            max_len = CTX_TOOL_RESULT_MAX_CHARS if is_tool_result else CTX_MSG_MAX_CHARS
        if is_latest:
            # 当前这一轮的输入按总预算走，不做单条硬截断
            max_len = max(max_len, budget)
        if len(text) > max_len:
    ...
    if tools and messages:
        ...
        if first_user:
            ...
            first_short = first_text[:CTX_TASK_RECAP_CHARS] + ("...[original task truncated]" if len(first_text) > CTX_TASK_RECAP_CHARS else "")
    ...
    if tools and messages:
        ...
        if latest_user:
            ...
            if latest_text:
                latest_short = latest_text[:CTX_LATEST_RECAP_CHARS] + ("...[latest task truncated]" if len(latest_text) > CTX_LATEST_RECAP_CHARS else "")
